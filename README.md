...
Two selectable modes, switchable via a dropdown in the header (switching deals a fresh board under the new mode — it doesn't try to hot-swap mid-game): **Classic (1–9)**, pairing on sum-to-10, faithful to the reference "10000" game, and the default; and **Extended (0–9)**, pairing on sum-to-9, Matt's own variant. Both use the same 9-column board, line-of-sight rules, scoring, and Add mechanic — only the digit range and pair target differ.
